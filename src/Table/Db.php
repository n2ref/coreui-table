...
class Db extends Table {
    ...
    public function __construct($resource) {
        ...
        $db = CoreUI\Registry::getDbConnection();
                            ...
                            if ( ! empty($data)) {
                                foreach ($data as $checkbox_value => $checkbox_title) {
                                    ...
                                    $is_checked = is_array($control_value) && in_array($checkbox_value, $control_value)
                                        ? 'checked="checked"'
                                        : '';
                                    $tpl->toolbar->field->checkbox->assign("[IS_CHECKED]", $is_checked);
                                    $tpl->toolbar->field->checkbox->reassign();
                                }
                            }
                            break;
                ...
                if ($column->isSorting()) {
                    if (isset($this->sessData['order']) && $this->sessData['order'] == $key + 1) {
                        if ($this->sessData['order_type'] == "ASC") {
                            $tpl->header->cell->sort->order_asc->assign('[SRC]', $this->theme_src . '/html/img/asc.gif');
                        } elseif ($this->sessData['order_type'] == "DESC") {
                            $tpl->header->cell->sort->order_desc->assign('[SRC]', $this->theme_src . '/html/img/desc.gif');
                        }
                    }
                    ...
                    if ($column instanceof Column) {
                        ...
                        switch ($column->getType()) {
                            ...
                            case 'status':
                                ...
                                if ($value == 'Y' || $value == 1) {
                                    $img = "<img src=\"{$this->theme_src}/html/img/lightbulb_on.png\" alt=\"[#on#]\" title=\"[#on#]/[#off#]\"
                                                 data-value=\"{$value}\"{$switch_active}/>";
                                } else {
                                    $img = "<img src=\"{$this->theme_src}/html/img/lightbulb_off.png\" alt=\"[#off#]\" title=\"[#on#]/[#off#]\"
                                                 data-value=\"{$value}\"{$switch_active}/>";
                                }
