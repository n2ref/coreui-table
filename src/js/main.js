
import coreuiTable from "./coreui.table";
import './coreui.table.instance';
import './coreui.table.templates';
import './coreui.table.utils';

import "./lang/ru";
import "./lang/en";

import "./controls/link";
import "./controls/button";
import "./controls/dropdown";
import "./controls/button_group";
import "./controls/custom";
import "./controls/page_size";
import "./controls/page_jump";
import "./controls/pages";
import "./controls/total";
import "./controls/search";
import "./controls/columns";

import "./filters/clear";
import "./filters/text";
import "./filters/number";
import "./filters/date";
import "./filters/datetime";
import "./filters/date_month";
import "./filters/date_range";
import "./filters/datetime_range";
import "./filters/checkbox";
import "./filters/radio";
import "./filters/select";
import "./filters/switch";

import "./search/text";
import "./search/number";
import "./search/date";
import "./search/date_month";
import "./search/datetime";
import "./search/date_range";
import "./search/datetime_range";
import "./search/checkbox";
import "./search/radio";
import "./search/select";
import "./search/switch";

import "./columns/date";
import "./columns/datetime";
import "./columns/html";
import "./columns/number";
import "./columns/numbers";
import "./columns/select";
import "./columns/switch";
import "./columns/text";

export default coreuiTable;